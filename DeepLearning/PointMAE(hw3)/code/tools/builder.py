...
def dataset_builder(args, config):
    # 数据集和数据加载器：dataset_builder函数根据给定的数据集配置创建一个PyTorch DataLoader。
    # 如果指定了分布式训练，它会处理分布式训练。
...
# 创建一个优化器（例如Adam、SGD）和一个调度器（例如LambdaLR、CosineLR），用于在训练过程中调整学习率。
...
# resume_model和resume_optimizer函数分别加载保存的模型权重和优化器状态，以便从检查点恢复训练。
